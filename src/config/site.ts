import { SiteSettings } from '@/types';

export const SITE_CONFIG: SiteSettings = {
  brandName: 'Dream to Achievers',
  companyLegalName: 'Dream to Achievers Global Network (Pvt) Ltd',
  supportEmail: 'support@dreamtoachievers.com',
  whatsappNumber: '+92 305 4511395',
  disclaimerText:
    'Example only. Actual product prices, partner purchase prices, selling prices, profit margins, eligibility conditions, and milestone rewards may vary according to the specific product catalog and company terms. Dream to Achievers does not make guaranteed-income claims.',
  communityRuleDescription:
    'A qualifying community member is a directly verified, registered user who has joined using your unique referral link and activated their partner account.',
  isReferralSignupEnabled: true,
};

export const NAV_LINKS = [
  { label: 'Home', href: '/' },
  { label: 'About', href: '/about' },
  { label: 'How It Works', href: '/how-it-works' },
  { label: 'Products', href: '/products' },
  { label: 'Ranks', href: '/ranks' },
  { label: 'Services', href: '/services' },
  { label: 'Contact', href: '/contact' },
];
