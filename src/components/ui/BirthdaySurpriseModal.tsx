...
import { Gift, Sparkle, X, Heart, Rocket, Trophy } from '@phosphor-icons/react';
...
export const BirthdaySurpriseModal: React.FC = () => {
  ...
  // Check trial count on load
  useEffect(() => {
    try {
      const count = parseInt(localStorage.getItem('faria_bday_trials_count') || '0', 10);
      setTrialsCount(count);
      // Auto-destroy if already opened 2 or more times
      if (count < 2) {
        setIsOpen(true);
      }
    } catch (e) {
  ...
  const handleOpenGift = () => {
    const nextCount = trialsCount + 1;
    setTrialsCount(nextCount);
    try {
      localStorage.setItem('faria_bday_trials_count', nextCount.toString());
    } catch (e) {}
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#080b11]/95 backdrop-blur-3xl animate-in fade-in duration-500">
      ...
      <div className="relative z-10 max-w-xl w-full text-center space-y-6 p-8 rounded-[2.5rem] bg-[#0c111d]/90 border border-white/15 backdrop-blur-2xl shadow-[0_0_80px_rgba(0,240,255,0.2)] overflow-hidden">
        ...
        {/* Top Trial Auto-Destruct Indicator Badge */}
        <div className="flex items-center justify-between text-xs font-mono text-slate-400 pb-2 border-b border-white/10">
          <span className="flex items-center space-x-1.5 text-amber-400 font-semibold">
            ...
            <span>Surprise Trail #{trialsCount + 1} of 2</span>
          </span>
          <span className="text-[11px] text-slate-500">
            {trialsCount >= 1 ? 'Last trail before auto-destruct' : '1 trail remaining'}
          </span>
        </div>
        ...
        {!isGiftOpened ? (
          ...
          <div className="space-y-7 py-4">
            ...
            <div className="relative w-32 h-32 mx-auto flex items-center justify-center">
              ...
              <div className="relative w-28 h-28 rounded-3xl bg-[#0e1424] border border-white/20 shadow-2xl flex items-center justify-center group cursor-pointer hover:scale-105 transition-transform" onClick={handleOpenGift}>
            ...
            <div className="space-y-2">
              <h2 className="text-2xl sm:text-3xl font-extrabold text-white font-heading tracking-tight">
                Special Gift for <span className="text-gradient-cyan">Faria Imran</span> 🎂
              </h2>
          ...
          <div className="space-y-6 py-2 animate-in zoom-in-95 duration-500">
            ...
            <div className="w-16 h-16 rounded-full bg-gradient-to-r from-[#00f0ff]/20 to-[#ff2a85]/20 border border-[#00f0ff]/40 flex items-center justify-center mx-auto text-[#00f0ff] shadow-xl">
              <Sparkle size={36} weight="fill" className="animate-bounce" />
            </div>
