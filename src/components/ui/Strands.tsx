...
export const Strands: React.FC<StrandsProps> = ({
  colors = ['#52B788', '#B8862E', '#2D6A56', '#FAF7EF'],
  count = 3,
  speed = 0.5,
  amplitude = 1,
  waviness = 1,
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('WebGL not supported for Strands effect', e);
      return;
    }
    ...
    return () => {
      ...
      if (ctn && renderer && renderer.gl.canvas.parentNode === ctn) {
  ...
  }, [speed, amplitude, waviness, thickness, glow, taper, spread, hueShift, intensity, opacity, scale, saturation, glass, refraction, dispersion, glassSize]);
