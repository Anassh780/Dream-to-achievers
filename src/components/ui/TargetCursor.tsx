...
export const TargetCursor: React.FC<TargetCursorProps> = ({
  targetSelector = '.cursor-target, button, a, .flip-card-wrapper, .border-glow-card',
  spinDuration = 2,
  ...
  useEffect(() => {
    ...
    const scrollHandler = () => {
      if (isScrollTicking || !activeTarget || !cursorRef.current) return;
  ...
  useEffect(() => {
    if (!enabled || isMobile || !cursorRef.current || !spinTl.current) return;
